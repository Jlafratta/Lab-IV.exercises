...
<body>
    ...
    echo "Key a buscar en arrayAssociative : " . $keyValue;
    ...
    function keyExists($key, $array)
    {
        ...
        foreach ($array as $x => $x_value) {
            if ($x == $key) {
                echo "Key $key encontrada. Value : $x_value";
                $flag = 1;
            }
        }
        if ($flag == 0) {
            echo "La Key $key no se encuentra en el arreglo";
        }
    }

    echo "<br><br>";
    echo keyExists($keyValue, $arrayAssociative);
    ...
    {
        return $keys = implode(",", array_keys($array));
    }

    echo "Keys de arrayAssociative : " . keysToArray($arrayAssociative);
